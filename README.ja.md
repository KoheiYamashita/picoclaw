...
| `queue_messages` | `false` | `CLAWDROID_AGENTS_DEFAULTS_QUEUE_MESSAGES` | 新しいメッセージを処理中の応答完了まで待機させる |
| `show_errors` | `true` | `CLAWDROID_AGENTS_DEFAULTS_SHOW_ERRORS` | エラーメッセージをチャットに表示 |
| `show_warnings` | `true` | `CLAWDROID_AGENTS_DEFAULTS_SHOW_WARNINGS` | 警告メッセージをチャットに表示 |
...
#### Android アクションカテゴリ (`tools.android`)

各カテゴリは `tools.android.<category>.enabled` で有効/無効を切り替えできます。カテゴリ内の個別アクションも `tools.android.<category>.actions.<action>` で制御可能です。

| カテゴリキー | デフォルト | 説明 |
|------------|----------|------|
| `alarm.enabled` | `true` | アラーム・タイマーアクション |
| `calendar.enabled` | `true` | カレンダーイベントアクション |
| `contacts.enabled` | `true` | 連絡先アクション |
| `communication.enabled` | `true` | 電話、SMS、メールアクション |
| `media.enabled` | `true` | メディア再生制御 |
| `navigation.enabled` | `true` | 地図・ナビゲーションアクション |
| `device_control.enabled` | `true` | デバイスハードウェア制御 |
| `settings.enabled` | `true` | システム設定アクション |
| `web.enabled` | `true` | アプリ内ブラウザ・Web 検索 |
| `clipboard.enabled` | `true` | クリップボード操作 |
...
`base_url` で任意の OpenAI 互換エンドポイント（xAI、DeepSeek、Groq、OpenRouter、Ollama 等）を指定できます。
...
ClawDroid は 40 以上の組み込みツールを提供し、AI エージェントが自律的に使用します。
...
追加アクションはカテゴリごとに整理されており、各カテゴリは設定で有効/無効を切り替えできます。

| カテゴリ | アクション | 説明 |
|---------|-----------|------|
| **Alarm** | `set_alarm`, `set_timer`, `dismiss_alarm`, `show_alarms` | アラーム・タイマー管理 |
| **Calendar** | `create_event`, `query_events`, `update_event`, `delete_event`, `list_calendars`, `add_reminder` | カレンダーイベント管理 |
| **Contacts** | `search_contacts`, `get_contact_detail`, `add_contact` | 連絡先の検索・管理 |
| **Communication** | `dial`, `compose_sms`, `compose_email` | 電話、SMS、メール |
| **Media** | `media_play_pause`, `media_next`, `media_previous`, `play_music_search` | メディア再生制御 |
| **Navigation** | `navigate`, `search_nearby`, `show_map`, `get_current_location` | 地図・ナビゲーション |
| **Device Control** | `flashlight`, `set_volume`, `set_ringer_mode`, `set_dnd`, `set_brightness` | デバイスハードウェア制御 |
| **Settings** | `open_settings` | システム設定を開く |
| **Web** | `open_url`, `web_search` | アプリ内ブラウザ・Web 検索 |
| **Clipboard** | `clipboard_copy`, `clipboard_read` | クリップボード操作 |
...
| `skill` | スキルの一覧表示・読み込み |
| `user` | ユーザーディレクトリ管理（マルチユーザープロファイル） |
...
- **音声モード** - 継続的な音声会話ループ（聞く → 送信 → 思考 → 話す）、リスニングの一時停止/再開に対応
...
ClawDroid は英語と日本語に対応しています。デバイスの言語設定に応じて UI テキストが自動的に切り替わります。Go バックエンドは WebSocket のクエリパラメータと HTTP の `Accept-Language` ヘッダーでロケールを受け取り、ステータスラベル・設定画面ラベル・警告メッセージを翻訳します。

新しい言語を追加するには:
- Go: `pkg/i18n/messages_*.go` に `register("xx", map[string]string{...})` を追加
- Android: `app/src/main/res/values-xx/strings.xml` と `feature/chat/src/main/res/values-xx/strings.xml` を作成
